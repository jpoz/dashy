...
.get('/events', function(req, res) {
  var self = this,
     ...
     fail_safe = setTimeout(function() {
       clearInterval(timer);
       res.send((req.query.jsonp || 'jsonp')+"(" + JSON.stringify({ revision: req.query.revision, list: [] }) + ");");
      }, 29999);
})

/*

/key/streetcar => {data:"json",timestamp:"dbtimestamp"}
/key/streetcar?timestamp=dbtimestamp => {data:"json",timestamp:"dbtimestamp"} 

*/

.get('/key/:dbkey', function(req, res) {
  var self = this;
  var fullkey  = "dashy:"+req.params.dbkey;
  
  timer = setInterval(function() {
    
    var client = $DB.createClient();
    client.get(fullkey+":timestamp", function(err, ts) {
      var timestamp = (req.query.timestamp || false).toString();
      var dbtimestamp = ts.toString();
      if (timestamp != dbtimestamp) {
        clearInterval(timer);
        client.get(fullkey, function(err, data) {
          var data = data.toString();
          res.contentType("text/javascript");
          res.send((req.query.jsonp || 'jsonp')+"(" + JSON.stringify({ timestamp: dbtimestamp, data: data }) + ");");
          client.close();
        })
      } else {
        client.close();
