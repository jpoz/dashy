...
$DB = {
  ...
  setValueFor: function(key, value) {
    var client = this.createClient();
    var jsons  = JSON.stringify(value);
    client.set("dashy:"+key, jsons, function(err, v) {
      console.log('New Value for: ', key, " => ", jsons);
      client.set("dashy:"+key+":timestamp", (new Date).valueOf(), function() {
        client.close();
      })
    })
  },
